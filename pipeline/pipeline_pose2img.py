...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image_mask_latents = image_mask_latents * self.vae.config.scaling_factor  # (b, 4, h, w)
        clip_vae_embeds = encoder_hidden_states

        if do_classifier_free_guidance:
            uncond_encoder_hidden_states = torch.zeros_like(clip_vae_embeds)
            uncond_class_labels = torch.zeros_like(class_labels)

            clip_vae_embeds = torch.cat([uncond_encoder_hidden_states, clip_vae_embeds], dim=0)
        ...
        image = self.vae.decode(latents.squeeze(2) / self.vae.config.scaling_factor, return_dict=False)[0]
